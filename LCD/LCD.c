...
{
    unsigned int i=0,j=0,k=0,l=0,n=0;
    i=num/10000;
    j=(num-10000*i)/1000;
    ...
    n=num%10;
    lcd_put_cur(x,y);
    if(i!=0)lcd_send_data(i+0x30);
    if((i!=0)||(j!=0))lcd_send_data(j+0x30);
    if((i!=0)||(j!=0)||(k!=0))lcd_send_data(k+0x30);
    if((i!=0)||(j!=0)||(k!=0)||(l!=0))lcd_send_data(l+0x30);
